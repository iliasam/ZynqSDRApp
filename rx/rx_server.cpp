...
void rx_server_remove(conn_t* c) {
    ...
    if (c->dx_has_preg_ident) {
        regfree(&c->dx_preg_ident);
        c->dx_has_preg_ident = false;
    }
    if (c->dx_has_preg_notes) {
        regfree(&c->dx_preg_notes);
        c->dx_has_preg_notes = false;
    }
...
conn_t* rx_server_websocket(websocket_mode_e mode, struct mg_connection* mc, u4_t ws_flags) {
    ...
    else if (sscanf(uri_ts, "no_wf/%lld/%256m[^\?]", &tstamp, &uri_m) == 2) {
        isKiwi_UI = true;
        isNo_WF = true;
    }
    else {
