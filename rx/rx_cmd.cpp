...
static void _dx_done(conn_t* conn, u4_t mark, u4_t max_quanta, int loop = 0, int nt_loop = 0, int send = 0, int nt_send = 0, int msg_sl = 0) {
    ...
    cprintf(conn, "DX_MKR DONE %.3f sec %s\n", max_quanta / 1e3,
            (loop || nt_loop || send || nt_send || msg_sl) ? stprintf("loop=%d|%d send=%d|%d msg_sl=%d", loop, nt_loop, send, nt_send, msg_sl) : "");
}
            ...
            if (badp == BADP_OK || badp == BADP_ADMIN_CONN_ALREADY_OPEN) {
                ...
                if (conn->auth == false) {
                    ...
                    if (stream_snd_or_wf || stream_admin_or_mfg) {
                        send_msg(conn, SM_NO_DEBUG, "MSG version_maj=%d version_min=%d platform=%d ext_clk=%d",
                                 version_maj, version_min, kiwi.platform, kiwi.ext_clk);
                    }
                    ...
                    if (stream_snd || (stream_wf && conn->isMaster) || stream_mon || stream_admin_or_mfg) {
                        ...
                        if (conn->foff_set && conn->foff != freq_offset_kHz) {
                            ...
                            else if (rx_count_server_conns(ADMIN_USERS) != 0) {
        ...
        if (kiwi_str_begins_with(cmd, "SET DX_UPD")) {
            ...
            if (gid >= -1 && gid < dx_db->actual_len) {
                if (func == DX_DEL && gid != -1 && freq == -1) {
                    ...
                    kiwi_asfree((void*)dxp->ident);
                    dxp->ident = strdup(text_m);
                    kiwi_asfree((void*)dxp->ident_s);
                    dxp->ident_s = kiwi_str_decode_inplace(strdup(text_m));
                    kiwi_asfree((void*)dxp->notes);
                    dxp->notes = strdup(notes_m);
                    kiwi_asfree((void*)dxp->notes_s);
                    dxp->notes_s = kiwi_str_decode_inplace(strdup(notes_m));
                    kiwi_asfree((void*)dxp->params);
...
#define TMEAS(x)
    ...
    case CMD_DX_FILTER:
        if (kiwi_str_begins_with(cmd, "SET DX_FILTER")) {
            ...
            kiwi_asfree(conn->dx_filter_ident);
            conn->dx_filter_ident = kiwi_str_decode_inplace(strdup(filter_ident_m));
            kiwi_asfree(conn->dx_filter_notes);
            conn->dx_filter_notes = kiwi_str_decode_inplace(strdup(filter_notes_m));
            ...
            if (conn->dx_filter_grep) {
                ...
                if (conn->dx_has_preg_ident) {
                    regfree(&conn->dx_preg_ident);
                    conn->dx_has_preg_ident = false;
                }
                if (conn->dx_has_preg_notes) {
                    regfree(&conn->dx_preg_notes);
                    conn->dx_has_preg_notes = false;
                }
    ...
    case CMD_MARKER:
        if (kiwi_str_begins_with(cmd, "SET MARKER")) {
            ...
            enum { DX_ADM_MKRS = 0,
                   DX_ADM_SEARCH_FREQ = 1,
                   DX_STEP = 2,
                   DX_ADM_SEARCH_IDENT = 3,
                   DX_MKRS = 4,
                   DX_ADM_SEARCH_NOTES = 5 };
            ...
            else if (sscanf(cmd, "SET MARKER db=%d dir=%d freq=%lf eibi_types_mask=0x%x filter_tod=%d",
                            &db, &dir, &min, &eibi_types_mask, &filter_tod) == 5) {
                func = DX_STEP;
            }
            else if (sscanf(cmd, "SET MARKER idx1=%d idx2=%d", &idx1, &idx2) == 2) {
                func = DX_ADM_MKRS;
                db = DB_STORED;
            }
            else if (sscanf(cmd, "SET MARKER search_freq=%lf", &freq) == 1) {
                func = DX_ADM_SEARCH_FREQ;
                db = DB_STORED;
            }
            else if (sscanf(cmd, "SET MARKER idx=%d search_ident=%64ms", &idx1, &ident) == 2) {
                func = DX_ADM_SEARCH_IDENT;
                db = DB_STORED;
            }
            else if (sscanf(cmd, "SET MARKER idx=%d search_notes=%64ms", &idx1, &notes) == 2) {
...
//#define DEBUG_DX_MKRS
    ...
    case CMD_GET_CONFIG:
        if (strcmp(cmd, "SET GET_CONFIG") == 0) {
            asprintf(&sb, "{\"r\":%d,\"g\":%d,\"s\":%d,\"pu\":\"%s\",\"pe\":%d,\"pv\":\"%s\",\"pi\":%d,\"n\":%d,\"m\":\"%s\",\"v1\":%d,\"v2\":%d,\"d1\":%d,\"d2\":%d,\"dna\":\"%08x%08x\"}",
                     rx_chans, gps_chans, net.serno, net.ip_pub, net.port_ext, net.ip_pvt, net.port, net.nm_bits, net.mac, version_maj, version_min, debian_maj, debian_min, PRINTF_U64_ARG(net.dna));
