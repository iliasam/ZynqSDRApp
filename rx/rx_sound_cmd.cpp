...
#define CW_OFFSET 0 // fixme: how is cw offset handled exactly?
    ...
    case CMD_NB_TYPE:
        if (sscanf(cmd, "SET nb type=%d en=%d", &n_type, &n_en) == 2) {
            ...
            if (n_type == NB_BLANKER) {
                switch (s->nb_algo) {
                case NB_STD:
                    m_NoiseProc_snd[rx_chan].SetupBlanker("SND", frate, s->nb_param[n_type]);
                    break;
                case NB_WILD:
                    nb_Wild_init(rx_chan, s->nb_param[n_type]);
                    break;
                }
            }
        }
    ...
    case CMD_NR_TYPE:
        if (sscanf(cmd, "SET nr type=%d en=%d", &n_type, &n_en) == 2) {
            ...
            switch (s->nr_algo) {
            case NR_WDSP:
                wdsp_ANR_init(rx_chan, (nr_type_e)n_type, s->nr_param[n_type]);
                break;
            case NR_ORIG:
                m_LMS[rx_chan][n_type].Initialize((nr_type_e)n_type, s->nr_param[n_type]);
                break;
            case NR_SPECTRAL:
                nr_spectral_init(rx_chan, s->nr_param[n_type]);
                break;
            }
        }
