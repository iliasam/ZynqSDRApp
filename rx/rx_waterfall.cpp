...
#define MAX_ZOOM (kiwi.wf_share ? 10 : 13)
    ...
    for (int winf = 0; winf < N_WF_WINF; winf++) {
        ...
        for (i = 0; i < WF_C_NSAMPS; i++) {
            ...
            case WINF_WF_BLACKMAN_HARRIS:
                window[i] *= (0.35875 - 0.48829 * cos((K_2PI * i) / (float)(WF_C_NSAMPS - 1)) + 0.14128 * cos((2.0 * K_2PI * i) / (float)(WF_C_NSAMPS - 1)) - 0.01168 * cos((3.0 * K_2PI * i) / (float)(WF_C_NSAMPS - 1)));
                break;
        ...
        if (n) {
            ...
            case CMD_SET_ZOOM: {
                ...
                if (kiwi_str_begins_with(cmd, "SET zoom=")) {
                    ...
                    else if (sscanf(cmd, "SET zoom=%d cf=%f", &_zoom, &cf) == 2) {
                        _zoom = CLAMP(_zoom, WF_ZOOM_MIN, MAX_ZOOM);
                        float halfSpan_Hz = (ui_srate / (1 << _zoom)) / 2;
                        cf *= kHz;
    ...
    if (wf->zoom <= 1) { // don't apply compensation when CIC not in use
        ...
        for (i = bin_dc_offset; i < wf->fft_used_limit; i++) {
            ...
            if (no_cic_comp) {
                ...
                re = ((float)fft->hw_fft[i][I]) * comp;
                im = ((float)fft->hw_fft[i][Q]) * comp;
            }
            pwr[i] = re * re + im * im;
            ...
            for (i = 0; i < wf->fft_used_limit; i++) {
                ...
                if (bin == _bin) {
                    ...
                    case WF_MAX:
                        ...
                        break; // gives dark lines
                    case WF_MIN:
                        if (p < pwr_out[bin]) pwr_out[bin] = p;
                        break; // gives bright lines
                    case WF_LAST:
                        pwr_out[bin] = p;
                        break; // using last value (random min/max) seems okay
                    default:
                        break;
                    }
                }
