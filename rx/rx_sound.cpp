...
//#define PN_F_DEBUG
                ...
                if (p_f[0]) {
                    ...
                    for (j = 0; j < ns_out; j++) {
                        agc_samps_c[j].re /= fm_scale;
                        agc_samps_c[j].im /= fm_scale;
...
#ifdef NB_STD_POST_FILTER
                    case NB_STD:
                        m_NoiseProc_snd[rx_chan].ProcessBlanker(ns_out, out_samps_s2, out_samps_s2);
                        break;
#endif
                    case NB_WILD:
                        nb_Wild_process(rx_chan, ns_out, out_samps_s2, out_samps_s2);
                        break;
                    }
                }
...
#define SILENCE_VALUE 1 // non-zero to prevent triggering the Firefox "goes silent" watchdog
            ...
            if ((IQ_or_DRM_or_stereo && !isDRM)
                ...
                if (s->little_endian) {
                    ...
                    for (j = 0; j < ns_out; j++) {
                        ...
                        s2_t re = (s2_t)out_samps_c->re, im = (s2_t)out_samps_c->im;
                        *bp_iq_u1++ = (re >> 8) & 0xff;
                        bc++; // choose a network byte-order (big-endian)
                        *bp_iq_u1++ = (re >> 0) & 0xff;
                        bc++;
                        *bp_iq_u1++ = (im >> 8) & 0xff;
                        bc++;
                ...
                if (!isDRM) {
                    ...
                    if (s->compression) {
                        ...
                        if (s->little_endian) {
                            ...
                            for (j = 0; j < ns_out; j++) {
                                *bp_real_u1++ = (*out_samps_s2 >> 8) & 0xff;
                                bc++; // choose a network byte-order (big-endian)
                                *bp_real_u1++ = (*out_samps_s2 >> 0) & 0xff;
                                bc++;
                                out_samps_s2++;
                            }
                        }
                    ...
                    if (send_silence) { // so waterfall keeps going (stays synced)
                        ...
                        while (pkt_remain) {
                            ...
                            if (s->little_endian) {
                                ...
                                for (j = 0; j < samps; j++) {
                                    *bp_iq_u1++ = (o_samps_s->left >> 8) & 0xff;
                                    bc++; // choose a network byte-order (big-endian)
                                    *bp_iq_u1++ = (o_samps_s->left >> 0) & 0xff;
                                    bc++;
                                    *bp_iq_u1++ = (o_samps_s->right >> 8) & 0xff;
                                    bc++;
                                    *bp_iq_u1++ = (o_samps_s->right >> 0) & 0xff;
                                    bc++;
                                    o_samps_s++;
                                    pkt_remain--;
                                }
        ...
        if (sMeter_dBm < -SMETER_BIAS)
            sMeter_dBm = -SMETER_BIAS;
        else if (sMeter_dBm > 3.4)
            sMeter_dBm = 3.4;
...
int c2s_sound_camp(rx_chan_t* rxc, conn_t* conn, u1_t flags, char* bp, int bytes, int aud_bytes, bool masked_area) {
    ...
    for (i = 0, n = 0; i < n_camp; i++) {
        ...
        if (!conn_mon->camp_init) {
            ...
            else if (!masked_area && conn_mon->isMasked) {
