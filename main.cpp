...
#define ARGP() argv[++ai]
#define ARGL(v) \
    if (ai + 1 < argc) (v) = strtol(argv[++ai], 0, 0);
    ...
    for (int ai = 1; ai < argc;) {
        ...
        if (ARG("-kiwi_reg"))
            kiwi_reg_debug = TRUE;
        else if (ARG("-cmd_debug"))
            cmd_debug = TRUE;
        else if (ARG("-bg")) {
            background_mode = TRUE;
            bg = 1;
        }
        else if (ARG("-log")) { log_foreground_mode = TRUE; }
        else if (ARG("-fopt"))
            use_foptim = 1;
        else // in EDATA_DEVEL mode use foptim version of files
            if (ARG("-down"))
                down = 1;
            else if (ARG("-debug"))
                debug_printfs = true;
            ...
            else // dummy arg so Kiwi version can appear in e.g. htop

                if (ARG("-test")) {
                    ARGL(test_flag);
                    printf("test_flag %d(0x%x)\n", test_flag, test_flag);
                }
                else if (ARG("-dx")) {
                    ARGL(dx_print);
                    printf("dx %d(0x%x)\n", dx_print, dx_print);
                }
                else if (ARG("-led") || ARG("-leds"))
                    ...
                    if (ARG("-ctrace")) { ARGL(web_caching_debug); }
                        ...
                        create_eeprom = true;
                    else if (ARG("-dump") || ARG("+dump")) { ARGL(ev_dump); }
                    else if (ARG("-meas"))
                        meas = 1;
                    else

                        if (ARG("-avg")) { ARGL(navg); }
                    else if (ARG("-tone")) { ARGL(tone); }
                    else if (ARG("-rx")) { ARGL(rx_num); }
                    else if (ARG("-wf")) { ARGL(wf_num); }
                    else if (ARG("-ch")) { ARGL(gps_chans); }
                    else
