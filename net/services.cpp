...
#define TZ_SERVER "googleapis.com"
        ...
        s = tzone_id;
        tzone_id = kiwi_str_encode(s);
        json_string_free(&cfg_tz, s);
        s = tzone_name;
        tzone_name = kiwi_str_encode(s);
        json_string_free(&cfg_tz, s);
...
#define PIPE_W 1
    ...
    if (child_pid == 0) {
        scall("C dup PIPE_R", dup2(si[PIPE_R], STDIN_FILENO));
        scall("C close PIPE_W", close(si[PIPE_W]));
...
static int _UPnP_port_open(void* param) {
    ...
    if (args->kstr != NULL) {
        ...
        else if (strstr(rp, "is redirected to")) {
            lprintf("UPnP: NAT port mapping in local network firewall/router created\n");
            rtn = 1;
        }
        else if (strstr(rp, "UPNP_DeletePortMapping() returned :")) {
...
#define N_IPINFO_RETRY 8
    int retry = 0;
    bool okay = false;
    do {
        i = retry % (N_IPINFO_RETRY / 2);
        if (i == 0)
            okay = ipinfo_json(1, "ipapi.co", "json", "ip", "latitude", "longitude");
        else if (i == 1)
            okay = ipinfo_json(1, "get.geojs.io", "v1/ip/geo.json", "ip", "latitude", "longitude");
        else if (i == 2)
            okay = ipinfo_json(0, "ip-api.com", "json", "query", "lat", "lon");
        else if (i == 3)
            okay = ipinfo_json(0, "whois.pconline.com.cn", "ipJson.jsp?json=true", "ip");
        retry++;
...
bool wakeup_reg_kiwisdr_com(wakeup_reg_e wakeup_reg) {
    ...
    else if (wakeup_reg == WAKEUP_REG_STATUS) {
